...
namespace PubNubMessaging.Core
{
	...
	public abstract class PubnubCore : INotifyPropertyChanged
	{
		#region "Events"
		...
		private bool InternetConnectionStatusWithUnitTestCheck (string channel, Action<PubnubClientError> errorCallback)
		{
			...
			{
				networkConnection = InternetConnectionStatus (channel, errorCallback);
		...
		protected virtual bool InternetConnectionStatus (string channel, Action<PubnubClientError> errorCallback)
		{
			bool networkConnection;
		...
		private void MultiChannelSubscribeInit<T>(ResponseType type, string channel, Action<T> userCallback, Action<T> connectCallback, Action<PubnubClientError> errorCallback)
		{
			...
			bool networkConnection = InternetConnectionStatusWithUnitTestCheck(channel, errorCallback);
			     ...
			     ) && (!overrideTcpKeepAlive))
			{
				    ...
				    && (asynchRequestState.Type == ResponseType.Subscribe || asynchRequestState.Type == ResponseType.Presence))
				{
					...
					else
					{
						...
						CallErrorCallback (PubnubErrorSeverity.Warn, PubnubMessageSource.Client,
						                   multiChannel, asynchRequestState.ErrorCallback, message, 
						                   PubnubErrorCode.NoInternetRetryConnect, null, null);
					}
	...
	#region "Crypto -- code split required"
	...
	#region "Webrequest and webresponse -- code split required"
	...
	#region "Json Pluggable Library -- code split required"
