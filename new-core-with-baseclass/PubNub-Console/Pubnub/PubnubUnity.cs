...
//TODO: replcae GoToCallback by CallErrorCallback
...
namespace PubNubMessaging.Core
{
	internal class PubnubUnity : PubnubCore
	{
		...
		{
			return ClientNetworkStatus.CheckInternetStatusUnity<T>(_pubnetSystemActive, errorCallback, channels, HeartbeatInterval);
		}
		...
		void Retry<T> (RequestState<T> currentState, string channel)
		{
			...
			string retryMessage = string.Format ("[0, \"Detected internet connection problem. Retrying connection attempt {0} of {1}\"]", _channelInternetRetry [channel], NetworkCheckMaxRetries);
			LoggingMethod.WriteToLog (string.Format("DateTime {0} {1} channel = {2} _urlRequest - Internet connection retry {3} of {4}", DateTime.Now.ToString(), currentState.Type, string.Join(",", currentState.Channels), _channelInternetRetry[channel], NetworkCheckMaxRetries), LoggingMethod.LevelError);
		...
		void OnPubnubHeartBeatTimeoutCallbackUnity<T>(System.Object heartbeatState)
		{
			...
			if (currentState != null)
			{
				...
				if(!networkConnection){
					...
					} else if (_channelInternetRetry [channel] == NetworkCheckMaxRetries) {
